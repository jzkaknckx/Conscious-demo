...
class FoveatedGraphMemory:
    ...
    def run_one_step(self, retina_out, msfb_out, current_fix: Tuple[float,float],
        ...
        """
        Revised run_one_step per user request:
        - Retrieval stage unchanged: retrieval-induced saccades remain non-recorded.
        - If retrieval succeeds -> strengthen mode (as before).
        - If retrieval fails -> enter learning mode:
            * perform recorded learning saccades guided by inverted-attention
            * LOOP until BOTH loop_closed == True AND learn_saccades >= self.max_learn_saccades
            * after loop exit: **unified ingestion** of attention-marked region (single call),
                reset attention, return {'mode':'learn', ...} (no extra re-check)
        """
        ...
        # If matched -> strengthen mode (unchanged behavior)
        if found_match:
            self.mode = 'strengthen'
            self.strengthen_and_promote(match_info)
            # do a small number of non-recorded refinement saccades (optional)
            for _ in range(self.reinforce_saccades):
                if chosen_move is None: break
        ...
        while not (loop_closed or learn_saccades >= self.max_learn_saccades):
            ...
            # incremental ingest local features around this fixation (still fine-grained)
            ...
            print(local_current)
            # check loop closure; this may reset attention (implementation detail)
            if self.attn.check_loop_and_reset(loop_radius=self.loop_radius):
                loop_closed = True

            # Safety: if attention reset cleared visited, but loop_closed true and we have reached threshold,
            # loop will exit naturally; otherwise continue until both conditions true.
            # Also allow early exit if some large global stop condition is desired (not implemented here).

        # Once loop_closed AND sufficient saccades performed, perform unified ingestion of the attention-marked region
        # Note: because check_loop_and_reset may have already reset attention, ensure we ingest features using any stored visited
        # positions or simply re-ingest from the current attention (practical compromise).
        # Here we ingest from the current (post-reset) attention map area using ingest_features_from_attention.
        unified_new_nodes = []
        try:
            ...
            # fallback: do nothing, but still return
            unified_new_nodes = []

        # Reset attention (explicitly) and set mode to idle or 'learn' as requested (return 'learn')
        self.attn.reset()
        self.mode = 'learn'
        # Return collected nodes information; by design we do NOT re-run retrieval here.
    ...
    def set_match_params(self, *,
                        min_inliers: int = 6,
                        edge_preserve_threshold: float = 0.3,
                        inlier_distance_thresh: float = None,
                        score_weights: Optional[Dict[str, float]] = None):
        """
        Configure matching sensitivity used in geometric_verify and final match decision.

        Args:
        min_inliers: minimum inlier count required by RANSAC to consider candidate
        edge_preserve_threshold: minimum fraction of preserved edges required for robust match
        inlier_distance_thresh: override RANSAC inlier distance threshold (pixels); None means keep default
        score_weights: dict of weights for scoring components, e.g. {'inlier':1.0, 'edge':2.0, 'persistence':1.5}
        """
        self.match_params = getattr(self, 'match_params', {})
        ...
        if inlier_distance_thresh is not None:
            self.match_params['inlier_distance_thresh'] = float(inlier_distance_thresh)
            self.ransac_inlier_thresh = float(inlier_distance_thresh)
        if score_weights is not None:
            self.match_params['score_weights'] = dict(score_weights)
        else:
            # reasonable default scoring weights
            self.match_params.setdefault('score_weights', {'inlier': 1.0, 'edge': 2.0, 'persistence': 1.0})
