...
import os
device = os.environ.get("CHECK_DEVICE", "cpu")
...
x=(torch.from_numpy(np.asarray(im).copy()).permute(2,0,1)[None].float()/255).to(device)
retina=RetinaModel(cropped_size=256,output_size=256,edge_apply_gaussian=True,edge_gauss_kernel_size=5,edge_gauss_sigma=1.).eval().to(device)
bank=MultiScaleFeatureBank([1,2,4]).eval().to(device)
def stats(t):
...
with torch.no_grad():
 ...
 curv,aps,ori,confidence=bank(cropped,precomputed_derivs={'grad':grad},return_confidence=True)
 cfg=configure_orientation_contract(MemoryConfig(),3);cfg.device=torch.device(device);cfg.H=cfg.W=256
 inputs=prepare_feature_subspaces({'grad':grad,'rgb':cropped,'curvature':curv,'aspect':aps,'orientation':ori,'orientation_confidence':confidence},cfg)
 out={'image':path,'device':device,'raw':{n:[stats(t[:,i:i+1]) for i in range(t.shape[1])] for n,t in [('curv',curv),('aps',aps),('ori',ori),('confidence',confidence)]}}
 ...
 out['bindings']=[{'id':r.region_id,'modality':r.modality_id,'parent':r.parent_region_id,'pixels':len(r.pixels),'samples':len(r.samples),'reasons':r.reasons} for r in report.regions]
 ...
 open('results/edge_binding_20260918/diagnostics_'+device.replace(':','_')+'.json','w').write(json.dumps(out,indent=2))
