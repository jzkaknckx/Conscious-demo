...
## 2026-09-21 长期训练与高级属性分区修正

详见 [长期连续训练改进与诊断方案](Alg_LongTermSupervisedTraining.md)。当前默认采用 grad 连续性响应门控后，curv、aps、ori 各自独立执行连续性分区，取代旧 aps/ori 复制 grad 分区规则；旧模式仅保留作对照。报告区分已实现的门控/诊断/执行窗口接口和待验证的长期学习算法。


## 2026-09-21 最新分区与 aps 语义修正

详见 [grad 区域细分与形状属性](Alg_RegionRefinementAndShape.md)。默认进一步改为 grad_refined：只在已完成 grad 父区域的拓扑内按属性细分。旧 aps 是梯度各向异性，真正区域长宽比在分区之后计算；已提供 RGB 区域几何诊断，尚未替换长期图的 aps 合同。


## 2026-09-21 最新 Notebook 检索与规模成本审查

详见 [运行结果与改进细则](../record/2026-09-21_notebook_retrieval_and_scaling_review.md)。最新运行仍显式使用 grad_gated；优先定位写入后原图检索失败，再补齐来源 family 和成员质量。建议按 observation 共享区域对应、批量实体精验，并给监督区域匹配接入 L2 先验；区分等价优化与近似候选策略。以上为待实施、待验证建议。


## 2026-09-21 检索与监督正确性修正

实现及验证见 [检索闭环与来源 family 修正](../record/2026-09-21_recall_family_correctness_implementation.md)。检索候选采用空间覆盖事件、角色加权中心和跨模板分轮保留；稀疏实体未命中时可由输入区域锚点补充候选，不使用标签或写入原点。来源 family 统一关联 grad 及细分属性，成员质量拆分并归一化同模态大小收益；区域歧义只在唯一实体几何证据足够时消解。性能改进暂缓；曲线拓扑重分解和特征语义替换不包含在本轮。


## 2026-09-21 修正后批量学习审查

详见 [最新结果与后续方案](../record/2026-09-21_postcorrection_learning_review.md)。两图 CUDA 原图检索成立，但最新批量仅提交 15/34 个目标，19 个均因局部歧义且无已验证实体而停止。建议分离局部模板复用与标注对象准入，保存未决关联但不伪增竞争模板证据；持续传播几何/采样质量，改用来源 family 覆盖并容忍属性子区域分裂。以上为待实施方案，本轮仅审查，性能优化继续搁置。


## 2026-09-21 标注准入与质量传播实现

详见 [实现与验证记录](../record/2026-09-21_admission_quality_implementation.md)。局部复用未决允许保存本地标注观察，不伪增竞争模板证据；提交前验证写入几何，不通过时至多一次本地重建。来源根约束 family 权重，成员质量持续参与精验，覆盖按来源根计算；分支轮廓分解为不重复边路径采样。新增冻结训练回查与可配置补充检索。监督合同升级 v2、采样合同 v4；属性区间多对多配准、未决实体自动巩固与性能优化不在本轮完成范围。


## 2026-09-21 大规模训练准备度

详见 [最新学习结果与准备度审查](../record/2026-09-21_large_training_readiness_review.md)。最新池完成 34/34 写入，冻结早中晚三点抽查命中，但 31 个实体关联仍未决且无跨观察实体更新。下一步需分开几何修复历史与有效 pending，补齐确认/驳回/过期退出和来源去重；完成全体回查、负例及独立观察巩固后再分段扩容。本次仅审查与只读抽查，未改模型。


## 2026-09-22 博士研究计划整理

研究计划见 [面向持续视觉学习的可解释层级超图记忆与空间检索](../research/PhD_Research_Proposal.md)。综合全部算法文档，区分已有原型、实验证据和未来研究；以跨观察对应与巩固、部分可见结构检索、增长图判别为主线。新增的完整训练回查记录显示 graph_version=34 的 34 个对象全部命中目标实体，且均启用结构补充检索；该结果属于训练记忆保持性，不代表留出集泛化。仅整理文档，未改变运行算法。


## 2026-09-22 记忆保持与泛化实验

详见 [第 8、9 节结果审查](../record/2026-09-22_recall_generalization_experiments.md)。最终池 34/34 原图实体回查通过，但已测 validation/test 共 57 个对象全部未决。下一步分离候选召回、结构精验与类别读出拒绝，建立同对象扰动阶梯，再验证共享部件、稳定核心及弱结构分类证据。以上为分析建议，本次未修改模型或重训。


## 2026-09-22 熟悉度与可靠差异驱动学习（讨论稿）

详见 [算法流程与更新规则](Alg_FamiliarityGuidedGraphLearning.md)。以可靠熟悉 family 建立局部对应，分别计算熟悉、可靠差异和未知状态；差异优先建档，旧关系仅在同实体关联可靠时更新。双向覆盖、空间锚定和关键冲突共同决定原实体变体、新组合或暂缓关联，不能仅按熟悉像素比例决策。学习幅度与独立证据计数分离，保护共享 GmemII 和旧观察；通过 pending 的确认/驳回/过期状态完成巩固。此为新设计，未修改模型。

补充收益判断见 [熟悉度驱动方案第 12 节](Alg_FamiliarityGuidedGraphLearning.md#12-相对收益预测与是否继续投入)：优先进行冻结记忆的局部对应/差异归因验证，不预测未经实测的准确率收益；达到正负例判定条件后再分阶段接入自动更新。


## 2026-09-22 泛化诊断与熟悉度先导实现

详见 [实现与小样本结果](../record/2026-09-22_generalization_diagnostics_familiarity_pilot.md)。补齐逐对象失败诊断、弱 L3 证据分类头对照、pending 状态管理、漏检与恢复检查；实现冻结图的 family 熟悉/可靠差异/未知热图。单对象六条件测试中原图/平移/亮度命中，但缩放 0.95 和遮挡均失去可靠对应，优先补齐尺度与部分可见配准。当前热图仅覆盖模板支持，尚未实现反向覆盖和非均匀图更新，不将增强输入计作独立观察或自动合并共享节点。Notebook 默认开启全流程。


## 2026-09-23 局部联合分配与诊断一致性

实现说明与验证见 [局部联合分配报告](../record/2026-09-23_joint_assignment_implementation.md)。空间精验对冲突槽位按连通分量进行 top-K 有界束搜索，保留证据互斥和原几何/覆盖/分数门限；没有受限解时仍未决，不自动删除角色。熟悉度探针区分局部熟悉与身份验证，未通过结构精验不建议强化。已知尺度变换只进入离线诊断，不进入正常检索或图更新。分类头保存拟合集独占的标准化参数、epoch=-1 基线和固定种子对照。真实汽车平移普通查询已恢复，亮度在已知位置精验恢复，缩放仍存在响应失配；尚未完成全面泛化验证。
